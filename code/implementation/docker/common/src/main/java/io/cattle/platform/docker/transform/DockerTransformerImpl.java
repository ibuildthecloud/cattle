...
public class DockerTransformerImpl implements DockerTransformer {
    ...
    public List<DockerInspectTransformVolume> transformVolumes(Map<String, Object> fromInspect) {
        ...
        Map<String, String> rw = rwMap(inspect.getVolumesRW());
        List<DockerInspectTransformVolume> volumes = new ArrayList<DockerInspectTransformVolume>();
        for (VolumeBind vb : volumeBinds) {
            String am = rw.containsKey(vb.getContainerPath()) ? rw.get(vb.getContainerPath()) : "rw";
            volumes.add(new DockerInspectTransformVolume(vb.getContainerPath(), vb.getHostPath(), am, binds.contains(vb
                    .getContainerPath())));
        }
        return volumes;
    }

    Map<String, String> rwMap(Volume[] volumeRws) {
        ...
        for (Volume v : volumeRws) {
            rwMap.put(v.getPath(), v.getAccessMode().toString());
        }
    ...
    public void transform(Map<String, Object> fromInspect, Instance instance) {
        ...
        // TODO Network, VolumesFrom, Links, SecurityOpt, ExtraHosts
    ...
    void setPorts(Instance instance, ExposedPort[] exposedPorts, Ports portBindings) {
        // TODO do we really need to populate the "ports" field?
        // There are two port related fields in Rancher: the user exposed "ports" and
        // the backend-only dockerPorts. Rancher already actually takes care of discovering all of a container's
        // ports and puts them into the dockerPorts field and creates port resources for them.
    ...
    void setCommand(Instance instance, String[] cmd) {
        // TODO We should refactor command and commandArgs down to one array field
        if (cmd == null) {
            cmd = new String[0];
        }
