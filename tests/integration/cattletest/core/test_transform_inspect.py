...
def test_transform_inspect_minimal(transform_url, admin_client):
    print admin_account
    print admin_client
    ...
    assert container['name'] == 'testName'
    ...
    assert len(container['commandArgs']) == 0
    assert len(container['dataVolumes']) == 0
    assert len(container['data']['fields']['ports']) == 0
...
def test_transform_inspect_full(transform_url, admin_client):
    inspect = inspect_payload('inspect1.json')
    ...
    assert container['cpuSet'] is None  # TODO Better test
    ...
    assert container['commandArgs'] == ['1', '2', '3']
    # TODO Cover entry point in different test
    assert len(container['dataVolumes']) == 2
    assert '/tmpcontainer2' in container['dataVolumes']
    assert '/tmp:/tmpcontainer' in container['dataVolumes']
    # Ports is suppressed as a field because itis also a link
    ports = container['data']['fields']['ports']
    assert len(ports) == 1
    assert '2222:1111/tcp' in ports  # TODO More port tests
...
def test_transform_inspect_rountrip(transform_url, admin_client):
    inspect = inspect_payload('inspect_roundtrip.json')
    response = requests.post(transform_url, inspect,
                             auth=HTTPBasicAuth(admin_client._access_key,
                                                admin_client._secret_key))
    assert response.status_code == 200
    ...
    with open(os.path.join(RESOURCE_DIR,
                           'container_roundtrip.json')) as data_file:
    ...
    container['commandArgs'].insert(0, container['command'])
    foo = ' '.join(container['commandArgs'])
    assert foo == orig_container['command']
    orig_vols = {}
    ...
    # Rancher containers "fake" ports, so they aren't in the inspect
    # assert container['data']['fields']['ports'] == \
    # orig_container['data']['fields']['ports']
